...
def create_question(question_text, days):
    '''
    两个参数，一个是问卷的文本内容，另外一个是当前时间的偏移天数，
...
class QuestionViewTests(TestCase):
    ...
    def test_index_view_with_a_future_question(self):
        ...
        """
        create_question(question_text="未来的问题")
        response = self.client.get(reverse('polls:index'))
        self.assertContains(response, "没有可用的投票")
        self.assertQuerysetEqual(response.context['latest_question_list'], [])
    ...
    def test_index_view_with_two_past_question(self):
        ...
        index页面可以同时显示多个问卷。
        """
        create_question(question_text="过去的问题 1.", days=30)
        create_question(question_text="过去的问题 2.", days=30)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(
        response.context['latest_question_list'],
...
def test_was_published_recently_with_old_question(self):
    """
    日期超过一天将返回False。这里创建了一个30天前发布的实例。
    """
    time = timezone.now() - datetime.timedelta(days=30)
